...
Each species *i* can contribute to the existence of a *j* ecosystem service. Therefore, we can map the relation species $\times$ ecosystem services as follows: 


$$ S = \mathbf{E}\vec{x}$$
where, $\mathbf{s}$ is a matrix of $K \times S$ dimension, in which $K$ is the ecosystem services vector and $S$ is the species vector and it describes the effects of one individuals species to ecossystem service. For our purpose, we can deal with the effect of species *j* on ecosystem services *i*. The vector $\vec{x}$ is related to the state of species (present or absent, abundance...). Each entry $E_{ij}$ depicts the effect of one individual species $j$ on each ecosystem service $i$. The $E_{ij}$ value could be the raw value or the relative value of each species $i$ contribution to the ecosystem service. For our purpose, 
...
$$
 ...
 \end{cases}
 $$
Therefore, the ecosystem services values are depicted as:

$$
E_i^{(t)} = \sum_{i = 1}^{S}x_j^{(t)}e_{ij}
$$
the effects of species loss could be investigated as $\Delta{S} = S^{(t_0)} - S^{(t_n)}$. 
