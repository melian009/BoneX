...
## Meeting 08/12/2025
Bruno, Carlos, Paulinha and Marcelo @ Puxadinho IB - USP

 Robustness Challenge Discussion
 * Recap of the model considering the single framework and the two routes being taken by Paulinha and Bruno, identifying the main challenge which is **How robust is our framework to parameter choice**. We need to address this issue and be confident of the robustness of our model/paramemter choices. 
 
 * Paulinha is leading the route considering how processes encoded in our model framework lead to the emergence of empirical patterns, whereas Bruninho is leading the route that considers empirical patterns as a starting point and explores the consequences of our framework to these patterns.
 
 * How will the inference be done? -- in Paulinha's case, where we start from a random expectation with all possible interactions and use to model to "prune" interactions as a framework to understand processes shaping ecological systems. One route for inference would be the reproducibility of networks properties. In this case, we could do a PCA of the network metrics (initial ones and the output ones comparing with the empirical distribution) and see how parameter combinations (i.e, symmetry and shape of cost benefit functions) lead to properties that are similar to the ones observed in empirical systems. An expectation would be a PCA where empirical networks cluster together with respect to their properties, whereas random (initial) networks would cluster separately and the networks emerging from model output would cluster together with the empirical ones. Alternatively, we might not see a signal in there (see below an example of a PCA with simulated data to show how this analysis would look like)
 

```{r, PCA_example, echo=FALSE, out.width="85%"}
library(ggplot2)
set.seed(42)
# Cluster A points
cluster_a <- matrix(rnorm(40, mean = 5, sd = 0.8), ncol = 2)
# Cluster B points
cluster_b <- matrix(rnorm(40, mean = -4.5, sd = 2), ncol = 2)
# Undefined points (noise spread across the center)
undefined_points <- matrix(rnorm(20, mean = 0, sd = 4), ncol = 2)

# Combine into a single data frame
my_data <- as.data.frame(rbind(cluster_a, cluster_b, undefined_points))
colnames(my_data) <- c("Var1", "Var2")

# Create a grouping variable 
group_labels <- c(rep("Random", 20), rep("Empirical", 20), rep("Model Out", 10))
my_data$Group <- as.factor(group_labels)

# 2. Perform PCA
pca_result <- prcomp(my_data[, c("Var1", "Var2")], scale. = TRUE)

# 3. Prepare data frame for ggplot
pca_data <- as.data.frame(pca_result$x) # Extract PC scores
pca_data$Group <- my_data$Group         # Add the grouping variable

# 4. Plot the PCA 
ggplot(pca_data, aes(x = PC1, y = PC2, color = Group)) +
  geom_point(size = 3) + # Add points and color by the 'Group' variable
  # Manually set colors: defined groups get distinct colors, undefined gets grey
  scale_color_manual(values = c("Random" = "orchid4", # Blue color
                                "Empirical" = "goldenrod", # Gold color
                                "Model Out" = "grey50")) +
  ...
  # Optional: Add concentration ellipses to highlight the main areas
  stat_ellipse(data = subset(pca_data, Group %in% c("Random", "Empirical")), aes(color = Group), type = "norm", level = 0.6)
