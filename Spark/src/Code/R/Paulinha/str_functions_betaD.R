...
# Beta distribution
create_strCB_beta <- function(Aij, is.bipartite=FALSE, shape1, shape2,...){
  #if it's not binary, make it binary
  matrix_A <- Aij
  matrix_A <- as.matrix((matrix_A > 0) + 0)
  # If network is bipartite, convert to adjacency
  if(is.bipartite){
    matrix_A <- cbind(rbind(matrix(0,nodes_i,nodes_i), t(A)),
...
#         physiological cost
boolean_model <- function(Ni_core, Nj_core, Ni_periphery, Nj_periphery,
                          connectance_pc, alpha=0.0001,
  ...
  A <- build_corep_graph(Ni_core, Nj_core, Ni_periphery, Nj_periphery,
                         connectance_pc)
  
  Ni <- Ni_core + Ni_periphery
  Nj <- Nj_core + Nj_periphery
  ...
  B <- create_strCB_beta(A, shape1=shape1B, shape2=shape2B)
  C <- create_strCB_beta(A, shape1=shape1C, shape2=shape2C)
  
  # Adding a small physiological cost
  costs_p <- c(create_strCB_beta(c(1:(Ni+Nj)), shape1=shape1Cp, shape2=shape2Cp)*0.1)
