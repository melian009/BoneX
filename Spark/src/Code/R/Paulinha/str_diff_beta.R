...
# Case studies 
## 1. alpha < beta (with 0.01 < alpha < 0.99)
## Parameter combination
alphas <- expand_grid(a1 = seq(0.01, 0.99, by = 0.05), 
...
ggplot() +
  stat_function(fun = dbeta, args = list(shape1 = 0.01, shape2 = 1), aes(color = "Cp"), lwd=1.5) +
  stat_function(fun = dbeta, args = list(shape1 = 0.9, shape2 = 1), aes(color = "Cost"), lwd=2.5) +
  stat_function(fun = dbeta, args = list(shape1 = 0.99, shape2= 1), aes(color = "Benefit"), lwd=1.5) +
...
core <- 0.65 # percentage that are core
...
for(j in 1:nrow(setup)){
  ...
  for (i in 1:nsim) {
    model_res <- boolean_model(nspi_c, nspj_c, nspi_p, nspj_p,
                               connect, 
                               shape1C = pull(setup[j,1]), shape2C = 1, 
                               shape1B = pull(setup[j,2]), shape2B = 1,
...
beta_dist_df <- tibble(
  x = x_values,
  "a=0.01, b=1" = dbeta(x_values, 0.01, 1),
  "a=0.2, b=1" = dbeta(x_values, 0.2, 1),
  "a=0.5, b=1" = dbeta(x_values, 0.5, 1),
  "a=0.75, b=1" = dbeta(x_values, 0.75, 1),
  "a=0.95, b=1" = dbeta(x_values, 0.95, 1)
)
...
#ggsave("./figures/str_beta1.pdf", width = 7)
