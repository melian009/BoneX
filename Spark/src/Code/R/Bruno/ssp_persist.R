...
redes_unicas <- unique(resultados[, c("nome_rede", "n_species_total")])
mean(redes_unicas$n_species_total, na.rm = TRUE) 
sd(redes_unicas$n_species_total, na.rm = TRUE)
...
phisio = resultados %>%
  filter(param_nivel_BCe == "moderate",service_providers == "random", n_services == 5)
...
cp <- glmmTMB(
  cbind(n_species_final, n_species_total - n_species_final) ~ 
    sistema * param_Cp_multiplier + (1 | nome_rede/replicate),
  data   = phisio,
  family = binomial
)
...
null <- glmmTMB(
  cbind(n_species_final, n_species_total - n_species_final) ~ 
    param_Cp_multiplier + (1 | nome_rede/replicate),
  data   = phisio,
  family = binomial
)
...
resultados <- phisio
resultados = readRDS("C:/Users/bruno/OneDrive/Documentos/posdoc/Suíça/projeto/resultados_1DP_metricas_finais.rds")
resultados$param_Cp_multiplier = as.factor(resultados$param_Cp_multiplier)
#4.1 descriptive
...
fig_cond <- ggplot(df_cond,
  ...
  # Um painel por métrica
  facet_wrap(~ metrica, ncol = 3) +
  
  # Paleta Set2 para as 3 linhas
  scale_color_brewer(palette = "Set2",
                     name   = "Network metric value") +
  ...
  scale_x_continuous(breaks = c(0, 0.1, 0.3, 0.5)) +
...
pred_pc1 <- ggpredict(mod_analise1_binomial, terms = c("param_Cp_multiplier_f", "PC1 [-2, 2]"))
...
ggsave(
  filename = "C:/Users/bruno/OneDrive/Documentos/GitHub/BoneX/Spark/Data/Simulated/Bruno/model_run/results/fig_efeito_pc1_sp.tiff", # Extensão .tiff
  plot = fig_efeito_pc1,
...
ggsave(
  filename = "C:/Users/bruno/OneDrive/Documentos/GitHub/BoneX/Spark/Data/Simulated/Bruno/model_run/results/fig_efeito_pc2_sp.tiff", # Extensão .tiff
  plot = fig_efeito_pc2,
  width = 11,                       # 11 cm de largura dá o respiro ideal para a legenda lateral
  height = 10,                      # 10 cm de altura mantém o gráfico bem proporcional
  units = "cm",
...
# =============================================================================
# MODELO 4 — ESTRUTURA × CUSTO FISIOLÓGICO (INTERAÇÃO)
# Pergunta: o efeito das métricas estruturais muda com o nível de custo?
# =============================================================================
...
# -----------------------------------------------------------------------
# 1. GARANTIR QUE custo_cat ESTÁ NO ESTRUC
# -----------------------------------------------------------------------

estruc <- estruc %>%
  mutate(custo_cat = factor(param_Cp_multiplier,
                            levels = c("0", "0.1", "0.3", "0.5"),
                            labels = c("Cp = 0", "Cp = 0.1", "Cp = 0.3", "Cp = 0.5")))


# -----------------------------------------------------------------------
# 2. MODELO COM INTERAÇÃO — estrutura × custo
# -----------------------------------------------------------------------
...
forest_int <- extrair_coefs_perfeitos_mod4(mod4, niveis)
write.xlsx(forest_int, "C:/Users/bruno/OneDrive/Documentos/GitHub/BoneX/Spark/Data/Simulated/Bruno/model_run/results/ranking_importancia_VS_cost_sp_persist.xlsx" )
...
ggsave("C:/Users/bruno/OneDrive/Documentos/GitHub/BoneX/Spark/Data/Simulated/Bruno/model_run/results/fig_forest_structure_by_cost.tiff",
