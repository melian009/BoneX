...
environment=1; % 0 no  environment filtering, 1  environment filtering
flutuacao=1;  % 0 no  environment fluctuation, 1  environment fluctuation
S=29; %species richness
E= 1; %number of ecosystem services
dim=S+E; %matrix size
scenario=3 %two-layer network of plants and herbivores, 
% ecosystem services generated by the consumers
p=0.2; %connectance
...
for simulacoes=1:nsims
    ...
    %trophic layer - ecosystem service provided by the consumers
    if scenario==3
        %split the matrices in two
        layer1=round(S/2); %partition the species assemblage in two
        while nintp>0 
            x=randi(S-layer1); 
            y=randi(layer1);  
            if matrizp(layer1+x,y)==0
                matrizp(layer1+x,y)=1; %columns affect positively the rows
                matrizn(y,layer1+x)=1; %columns affect negatively the rows
                nintp=nintp-1;
            end
        end
        for i=layer1+1:S
